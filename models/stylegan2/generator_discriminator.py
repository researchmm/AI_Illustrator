...
class StyleGANv2Generator(nn.Module):
    ...
    def forward(self,
        ...
        # style mixing
            ...
            if random_generate and single_change_channel != -1:
                raise ValueError('Unmatched generate mode.')

            if random_generate:
                random_latent_num = 1
                # print(latent2.shape)
                for i in range(random_latent_num):
                    random_position = 14
                    random_latent = styles[0]
                    for j in range(512):
                        latent2[:, random_position, j] = random_latent[0, j]

            if single_change_channel != -1:
                latent = styles[0].unsqueeze(1).repeat(1, single_change_channel - 1, 1)
                mix_latent = styles[1].unsqueeze(1).repeat(1, 1, 1)
                latent2 = styles[0].unsqueeze(1).repeat(1, self.num_latents - single_change_channel, 1)
                latent2 = torch.cat([mix_latent, latent2], 1)
